...
namespace KrTrade.Nt.Console
{
    ...
    public class FileLogger : ILogger
    {
        ...
        public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            // Prepend log level
            var logLevelString = configuration.OutputLogLevel ? $"{logLevel.ToString().ToUpper()}: " : "";

            // Prepend the time to the log if desired
            var timeLogString = configuration.LogTime ? $"[{currentTime}] " : "";
            ...
            lock (fileLock)
            {
                ...
                // Open the file
                using (var fileStream = new StreamWriter(File.Open(filePath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite)))
                {
                    // Go to end
