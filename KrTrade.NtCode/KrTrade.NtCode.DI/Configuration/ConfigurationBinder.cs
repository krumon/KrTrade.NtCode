...
namespace KrTrade.NtCode.Configuration
{
    ...
    public static class ConfigurationBinder
    {
        ...
        private static void BindNonScalar(this IConfiguration configuration, object instance, BinderOptions options)
        {
            if (instance != null)
            {
                ...
                if (options.ErrorOnUnknownConfiguration)
                {
                    ...
                    {
                        throw new InvalidOperationException($"Error_MissingConfig, {nameof(options.ErrorOnUnknownConfiguration)}, {nameof(BinderOptions)}, {instance.GetType()}, {string.Join(", ", missingPropertyNames)}.");
                    }
                }
        ...
        private static object CreateInstance([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.NonPublicConstructors)] Type type)
        {
            ...
            {
                throw new InvalidOperationException($"Error_CannotActivateAbstractOrInterface, {type}).");
            }
            ...
            if (type.IsArray)
            {
                ...
                {
                    throw new InvalidOperationException($"UnsupportedMultidimensionalArray, {type}).");
                }
            ...
            if (!type.IsValueType)
            {
                ...
                {
                    throw new InvalidOperationException($"Error_MissingParameterlessConstructor, {type}).");
                }
            }
        ...
        private static bool TryConvertValue([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type type, string value, string path, out object result, out Exception error)
        {
            ...
            if (converter.CanConvertFrom(typeof(string)))
            {
                ...
                {
                    error = new InvalidOperationException($"Error_FailedBinding, path, type), {ex}.");
                }
                return true;
            }
