    ...
    public abstract class FileConfigurationProvider : ConfigurationProvider, IDisposable
    {
        ...
        private void Load(bool reload)
        {
            ...
            if (file == null || !file.Exists)
            {
                ...
                {
                    var error = new StringBuilder($"Error_FileNotFound, {Source.Path}.");
                    if (!string.IsNullOrEmpty(file?.PhysicalPath))
                    {
                        error.Append($"Error_ExpectedPhysicalPath, {file.PhysicalPath}.");
                    }
            ...
            {
                using (System.IO.Stream stream = OpenRead(file))
                {
                    try
                    {
                        Load(stream);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        var exception = new InvalidDataException($"Error_FailedToLoad, {file.PhysicalPath}, {ex}.");
