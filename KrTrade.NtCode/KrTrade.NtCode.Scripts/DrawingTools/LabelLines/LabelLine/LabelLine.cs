...
namespace NinjaTrader.NinjaScript.DrawingTools
{
    ...
    /// 
    /// Version			: v.0.2.
    /// Description		: Clase base para todas las herramientas que dibujan lineas con etiqueta. 
    ...
    ///					  La etiqueta podrá ser centrada o puesta en los márgenes derecho o izquierdo del gráfico por el usuario. 
    /// Improvements	: Se actualizan las propiedades del cuadro de diálogo de la herramienta.
    /// </summary>
    /// 
    //[CategoryExpanded("Label",true)]
    [Gui.CategoryOrder("Text", 21)] 
	[Gui.CategoryOrder("Label", 22)] 
	[Gui.CategoryOrder("Line", 23)] 
	public class LabelLine : DrawingTool
	{
		...
		/// <summary>
		/// The label area opacity.
		/// </summary>
		private int labelAreaOpacity;

		/// <summary>
		/// The label area color.
		/// </summary>
		private Brush labelAreaBrush;

		/// <summary>
		/// 
		/// </summary>
		private readonly DeviceBrush labelAreaBrushDevice = new DeviceBrush();
		...
		/// </summary>
		protected LabelLineType LabelType { get; set; }
		...
		[Browsable(false)]
		[XmlIgnore]
		protected Margin LabelMargin { get; set; }
		...
		[Browsable(false)]
		[XmlIgnore]
		protected Margin LabelTextMargin { get; set; }
		...
		/// </summary>
		[Display(ResourceType = typeof(ChartResources), GroupName = "GuiChartsCategoryData", Name = "Anchor property", Order = 1)]
		public ChartAnchor Anchor { get; set; }

		/// <summary>
		/// Indicates if the drawing tool shows the anchor text in the label.
		/// </summary>
		[Display(ResourceType = typeof(ChartResources), GroupName = "GuiChartsCategoryData", Name = "Visible", Description = "True if the anchor text is visible in the label.", Order = 2)]
		...
		/// </summary>
		[XmlIgnore]
		[Display(Name = "Description", Description = "The description of the label line", GroupName = "Text", Order = 1)]
		public string LabelText { get; set; }

		/// <summary>
		/// Represents the stroke properties of the label text.
		/// </summary>
		[Display(Name = "Text Color", Description = "The color of the text.", GroupName = "Text", Order = 3)]
		...
		/// The label area color.
		/// </summary>
		[Display(Name = "Area Color", Description = "The color of the label area." ,GroupName = "Label", Order = 4)]
		public Brush LabelAreaBrush
		{
			get { return labelAreaBrush; }
			set
			{
				labelAreaBrush = value;
				if (labelAreaBrush == null)
					return;
				if (labelAreaBrush.IsFrozen)
					labelAreaBrush = labelAreaBrush.Clone();
				labelAreaBrush.Opacity = labelAreaOpacity / 100d;
				labelAreaBrush.Freeze();
				labelAreaBrushDevice.Brush = null;
			}
		}

		/// <summary>
		/// Serializes the <see cref="LabelAreaBrush"/> property.
		/// </summary>
		[Browsable(false)]
		public string LabelAreaBrushSerialize
		{
			get { return Serialize.BrushToString(LabelAreaBrush); }
			set { LabelAreaBrush = Serialize.StringToBrush(value); }
		}
		...
		/// </summary>
		[Range(0, 100)]
		[Display(Name = "Area Opacity", Description = "The label area opacity.", GroupName = "Label", Order =5)]
		public int LabelAreaOpacity
		{
			get { return labelAreaOpacity; }
			set
			{
				labelAreaOpacity = Math.Max(0, Math.Min(100, value));
				labelAreaBrushDevice.Brush = null;
			}
		}
		...
		[Display(Name = "Area Color", Description = "The area color of the label", GroupName = "Label", Order = 4)]
		...
		/// Represents the stroke properties of the label line.
		/// </summary>
		[Display(Name = "Line Color", Description = "The color of the line and the label outline", GroupName = "Line", Order = 1)]
		...
		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				...
				Anchor = new ChartAnchor
				{
					IsEditing = true,
					DrawingTool = this,
					DisplayName = "Display FileName", //NtCore.Resources.Texts.LabelLineDisplayName,
					IsBrowsable = true,
				...
				LabelTextStroke = new Stroke(Brushes.White);
		...
		public override void OnRender(ChartControl chartControl, ChartScale chartScale)
		{
			...
			LabelTextStroke.RenderTarget = RenderTarget;
