...
namespace KrTrade.Nt.Services
{
    ...
    /// </summary>
    public class MinSeries : IndicatorSeries
    {

        /// <summary>
        /// Create <see cref="MinSeries"/> default instance with specified properties.
        /// </summary>
        /// <param name="input">The <see cref="IBarsService"/> instance used to gets <see cref="NinjaScriptBase"/> object necesary for <see cref="MinSeries"/>.</param>
        /// <param name="period">The period to calculate the cache values.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public MinSeries(IBarsService input, int period) : this(input?.High, period, input?.CacheCapacity ?? DEFAULT_CAPACITY, input?.RemovedCacheCapacity ?? DEFAULT_OLD_VALUES_CAPACITY, input?.Index ?? 0)
        {
        }
        ...
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public MinSeries(INumericSeries<double> input, int period, int capacity, int oldValuesCapacity, int barsIndex) : base(input, "MAX", period, capacity, oldValuesCapacity, barsIndex)
        {
        }
