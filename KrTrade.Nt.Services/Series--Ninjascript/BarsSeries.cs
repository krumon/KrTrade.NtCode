...
{
    public class BarsSeries : NinjaSeries<double,NinjaScriptBase>, IBarsSeries
    {
        ...
        /// </summary>
        /// <param name="input">The <see cref="IBarsMaster"/> instance used to gets <see cref="NinjaScriptBase"/> object necesary for <see cref="BarsCache"/>.</param>
        ...
        public BarsSeries(IBarsService input, int capacity = DEFAULT_CAPACITY, int oldValuesCapacity = DEFAULT_OLD_VALUES_CAPACITY) : this(input?.Ninjascript, input?.Index ?? 0, capacity, oldValuesCapacity)
        {
        }
        ...
        /// <param name="oldValuesCapacity">The length of the old values cache. This values are at the end of cache.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public BarsSeries(NinjaScriptBase input, int barsIndex, int capacity = DEFAULT_CAPACITY, int oldValuesCapacity = DEFAULT_OLD_VALUES_CAPACITY) : base(input, period: 1, capacity, oldValuesCapacity, barsIndex)
        {
            CurrentBar = new CurrentBarSeries(input, barsIndex, capacity, oldValuesCapacity);
            Time = new TimeSeries(input, barsIndex, capacity, oldValuesCapacity);
            Open = new HighSeries(input.Opens[_barsIndex], barsIndex, capacity, oldValuesCapacity);
            High = new HighSeries(input.Highs[_barsIndex], barsIndex, capacity, oldValuesCapacity);
            Low = new HighSeries(input.Lows[_barsIndex], barsIndex, capacity, oldValuesCapacity);
            Close = new HighSeries(input.Closes[_barsIndex], barsIndex, capacity, oldValuesCapacity);
            Volume = new VolumeSeries(input.Volumes[_barsIndex], barsIndex, capacity, oldValuesCapacity);
            Tick = new TickSeries(input, barsIndex, capacity, oldValuesCapacity);
        }
