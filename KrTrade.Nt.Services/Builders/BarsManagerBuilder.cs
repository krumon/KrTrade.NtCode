...
namespace KrTrade.Nt.Services
{
    ...
    public class BarsManagerBuilder : IBarsManagerBuilder
    {
        ...
        public IBarsManager Build(NinjaScriptBase ninjascript, Action<string, BarsPeriod, string> addDataSeriesDelegate = null)
        {
            ...
            IBarsServiceBuilder primaryBarsServiceBuilder = new BarsServiceBuilder(ninjascript,printService,addDataSeriesDelegate);
            _primaryBarsServiceDelegateBuilder?.Invoke(primaryBarsServiceBuilder);

            IBarsService primaryBarsService = primaryBarsServiceBuilder.Build();
            ...
            {
                IBarsServiceBuilder barsServiceBuilder = new BarsServiceBuilder(ninjascript,printService,addDataSeriesDelegate);
                barsServiceDelegateBuilder(barsServiceBuilder);
                
                IBarsService barsService = barsServiceBuilder.Build();
            ...
            if (barsManager.Count > 0)
                ...
                {
                    addDataSeriesDelegate?.Invoke(barsManager[i].InstrumentName, barsManager[i].BarsPeriod, barsManager[i].TradingHoursName);
                    // Log trace
