...
namespace KrTrade.Nt.Services
{
    ...
    public class AvgCache : IndicatorsCache
    {
        ...
        public AvgCache(IBarsService input, int period, int capacity = DEFAULT_CAPACITY, int oldValuesCapacity = DEFAULT_OLD_VALUES_CAPACITY, int barsIndex = 0) : this(input?.Ninjascript.Inputs[barsIndex], period, capacity, oldValuesCapacity)
        {
        }
        /// <summary>
        /// Create <see cref="AvgCache"/> default instance with specified properties.
        ...
        /// <param name="barsIndex">The index of NinjaScript.Bars used to gets cache elements.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public AvgCache(NinjaScriptBase input, int period, int capacity = DEFAULT_CAPACITY, int oldValuesCapacity = DEFAULT_OLD_VALUES_CAPACITY, int barsIndex = 0) : this(input?.Inputs[barsIndex], period, capacity, oldValuesCapacity)
        {
        }
        /// <summary>
        /// Create <see cref="AvgCache"/> default instance with specified properties.
        ...
        /// <param name="oldValuesCapacity">The length of the old values cache. This values are at the end of cache.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public AvgCache(ISeries<double> input, int period, int capacity = DEFAULT_CAPACITY, int oldValuesCapacity = DEFAULT_OLD_VALUES_CAPACITY) : base(input, period, capacity, oldValuesCapacity)
        {
