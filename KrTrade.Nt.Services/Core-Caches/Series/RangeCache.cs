...
namespace KrTrade.Nt.Services
{
    ...
    public class RangeCache : DoubleCache<ISeries<double>>
    {
        ...
        public RangeCache(IBarsService input, int capacity = DEFAULT_CAPACITY, int lengthOfRemovedCache = DEFAULT_LENGTH_REMOVED_CACHE, int barsIndex = 0) : this(input?.Ninjascript.Highs[barsIndex], input?.Ninjascript.Lows[barsIndex], capacity, lengthOfRemovedCache, barsIndex)
        {
        }
        ...
        /// <param name="barsIndex">The index of NinjaScript.Bars used to gets cache elements.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public RangeCache(NinjaScriptBase input, int capacity = DEFAULT_CAPACITY, int lengthOfRemovedCache = DEFAULT_LENGTH_REMOVED_CACHE, int barsIndex = 0) : this(input?.Highs[barsIndex], input?.Lows[barsIndex], capacity, lengthOfRemovedCache, barsIndex)
        {
        }
        ...
        /// <param name="lengthOfRemovedCache">The length of the removed values cache. This values are at the end of cache.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public RangeCache(ISeries<double> input1, ISeries<double> input2, int capacity = DEFAULT_CAPACITY, int lengthOfRemovedCache = DEFAULT_LENGTH_REMOVED_CACHE, int barsIndex = 0) : base(input1, capacity, lengthOfRemovedCache, barsIndex)
        {
