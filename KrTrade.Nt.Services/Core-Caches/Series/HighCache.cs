...
namespace KrTrade.Nt.Services
{
    ...
    public class HighCache : DoubleCache<ISeries<double>>
    {
        ...
        public HighCache(IBarsService input, int capacity = DEFAULT_CAPACITY, int lengthOfRemovedCache = DEFAULT_LENGTH_REMOVED_CACHE, int barsIndex = 0) : this(input?.Ninjascript.Highs[barsIndex], capacity, lengthOfRemovedCache, barsIndex)
        {
        }
        ...
        /// <param name="barsIndex">The index of NinjaScript.Bars used to gets cache elements.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public HighCache(NinjaScriptBase input, int capacity = DEFAULT_CAPACITY, int lengthOfRemovedCache = DEFAULT_LENGTH_REMOVED_CACHE, int barsIndex = 0) : this(input?.Highs[barsIndex], capacity, lengthOfRemovedCache, barsIndex)
        {
        }
        ...
        /// <param name="lengthOfRemovedCache">The length of the removed values cache. This values are at the end of cache.</param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="input"/> cannot be null.</exception>
        public HighCache(ISeries<double> input, int capacity = DEFAULT_CAPACITY,int lengthOfRemovedCache = DEFAULT_LENGTH_REMOVED_CACHE, int barsIndex = 0) : base(input, capacity,lengthOfRemovedCache, barsIndex)
        {
        }
