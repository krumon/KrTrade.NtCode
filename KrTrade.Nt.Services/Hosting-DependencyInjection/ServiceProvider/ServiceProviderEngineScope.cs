...
namespace KrTrade.Nt.Services
{
    internal sealed class ServiceProviderEngineScope : IServiceScope, IServiceProvider, IAsyncDisposable, IServiceScopeFactory
    {
        ...
        public object GetService(Type serviceType)
        {
            if (_disposed)
            {
                throw new Exception("ThrowObjectDisposedException");
            }
        ...
        public void Dispose()
        {
            ...
            if (toDispose != null)
            {
                for (int i = toDispose.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        throw new InvalidOperationException($"AsyncDisposableServiceDispose, {toDispose[i]}");
                    }
                }
            }
        }
        ...
        private static async ValueTask Await(int i, ValueTask vt, List<object> toDispose)
        {
